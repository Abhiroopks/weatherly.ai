...
def get_daily_weather(loc: Coordinates, days: int = 1) -> list[DailyWeather]:
    """
    Get the daily weather for the given location for the given number of days.

    This function will cache the result of the API call for up to 7 days.

    Args:
        loc (Coordinates): The location to get the weather for.
        days (int, optional): The number of days to get the weather for. Defaults to 1.

    Returns:
        list[DailyWeather]: A list of DailyWeather objects representing the weather
            conditions for the given location over the given number of days.
    """
    prefix: str = "daily"
    ...
    for day_offset in range(days):
        ...
        print(f"Looking up weather for {date}")
        full_prefix: str = f"{prefix}_{date_str}"
        # Check if the weather for this day is already in the cache.
        if WEATHER_CACHE.has_weather(full_prefix, loc):
            daily_weather: DailyWeather = WEATHER_CACHE.get_weather(full_prefix, loc)  # type: ignore
            ...
            for _day_offset in range(days):
                ...
                WEATHER_CACHE.add_weather(_full_prefix, loc, _daily_weather)
...
def get_current_weather(
    ...
    """
    Gets the current weather conditions for a list of locations.

    Args:
        locations (list[Coordinates]): The locations to get the current weather for.
        use_cache (bool, optional): Whether to use the cache when generating the weather report. Defaults to True.

    Returns:
        list[CurrentWeather]: A list of CurrentWeather objects, one for each location, containing the current weather conditions.
    """
    weather_data: list[CurrentWeather] = []
    prefix: str = "current"
